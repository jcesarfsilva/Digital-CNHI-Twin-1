...
def show_predict_page():
    st.markdown("<h1 style='text-align: center; color: black;'>Predição estática dos modelos multivariacionais</h1>", unsafe_allow_html=True)
    st.markdown("<h3 style='text-align: center; color: black;'>Modelos com 7 e 8 variáveis</h3>", unsafe_allow_html=True)
    tab1, tab2 = st.tabs(["📈 Dia ","📈 Arquivo"])
    with tab1:
        ...
        # imagem_statistic = next(( img for img in jpeg_files if f"static" in img), None)
        # print("8888888888")
        with col5:
            # st.image()
            
            # statisticimg = next(( img for img in jpeg_files if img.lower().startswith("statistic_") in img), None)
            # print ("555555555555555",statisticimg)
        
            # if imagem_correspondente:
                # st.image(jpeg_files[0], use_container_width=True)
            pass
        with col6:
            pass
            # st.image()
